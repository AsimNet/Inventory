...
public class InventoryProvider extends ContentProvider {
    ...
    public Uri insertProduct(Uri uri, ContentValues values) {
        ...
        String name = values.getAsString(InventoryContract.ProductEntry.COL_NAME);
        if (name == null) {
            throw new IllegalArgumentException("Product requires a name");
        }

        // Check that the gender is valid
        Integer quantity = values.getAsInteger(InventoryContract.ProductEntry.COL_QUANTITY);

        // If the weight is provided, check that it's greater than or equal to 0 kg
        Float price = values.getAsFloat(InventoryContract.ProductEntry.COL_PRICE);
    ...
    public int update(Uri uri, ContentValues contentValues, String selection, String[] selectionArgs) {
        ...
        switch (match) {
            case INVENTORY:
                rowsUpdated = database.update(InventoryContract.ProductEntry.TABLE_NAME,
                        contentValues,
                        selection,
                        selectionArgs);
                break;
